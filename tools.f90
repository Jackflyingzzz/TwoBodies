...
!****************************************************************************
!
subroutine pre_correc(ux,uy,uz,epsi)
!
!****************************************************************************
...
utin = utin/yly
utout = utout/yly
!! Write Mass balance to file with every snapshot
!if (mod(itime,imodulo).eq.0 .and. itime.ge.idebmod) then 
!  inquire(file="mass.txt", exist=exist)
!  if (exist) then
!    open(19, file="mass.txt", status="old", position="append", action="write")
!  else
!    open(19, file="mass.txt", status="new", action="write")
!  end if
!  write(19, *) utin-utout
!  close(19)
!endif  
...
if (epsi(nx,j,k).eq.0.) then
   bxxn(j,k)=bxxn(j,k)-utout+utin
!   bxxn(j,k)=bxxn(j,k)       ! To impose alanytical BC 
else
   bxxn(j,k)=bxxn(j,k)
endif  
enddo
...
if (iecoule.eq.7) then
   ...
   do i=1,nx
      ...
      ux(i,1,k)=0.
      uy(i,1,k)=0. 
      ux(i,ny,k)=16.*(x*x*x*x-2.*x*x*x+x*x)
      uy(i,ny,k)=0.
   enddo
   enddo
endif
